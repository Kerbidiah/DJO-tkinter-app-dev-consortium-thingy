...
class Todo(tk.Tk):
	def __init__(self, tasks=None):
		...
		self.tasksCanvas = tk.Canvas(self) # make a canvas inside the window
		self.tasksFrame = tk.Frame(self.tasksCanvas) # make a frame inside the canvas
		self.textFrame = tk.Frame(self) # make a frame

		# set up scroll bar														V  this is how it knows to size?
		...
		self.taskCreate = tk.Text(self.textFrame, height=3, bg="white", fg="black") # text input box

		self.tasksCanvas.pack(side=tk.TOP, fill=tk.BOTH, expand=1) # make canvas take up entire window
		self.scrollbar.pack(side=tk.RIGHT, fill=tk.Y) # make scroll bar on right side and fill y

		# ????
		self.canvasFrame = self.tasksCanvas.create_window((0,0),
							window=self.tasksFrame, anchor="n")

		# pack a bunch of stuff and set focus to text input
		self.taskCreate.pack(side=tk.BOTTOM, fill=tk.X)
		self.tasksFrame.pack(side=tk.BOTTOM, fill=tk.X)
		self.taskCreate.focus_set()

		# this sets up the first entry + keybind
		...
		# key bindings
		self.bind("<Return>", self.addTask) # for key bindings, DON'T PUT PARENTHESES AT END, we want the func itself, not the func to execute
